...
One way to define "multimodal understanding" could be that a model truly understands two modalities if, when they represent the same thing, it treats them the same way internally.A photo of the sentence _"mitochondria are the powerhouse of the cell"_ and the text itself carry the same meaning. A genuinely multimodal model should activate the same features for both, because it has recognised what they represent, not just what format they arrived in.

Qwen Scope gives us the tool to test this directly. By training a Sparse Autoencoder on each transformer layer of Qwen3.5 models, it exposes the feature circuits the model uses at every depth. We can look at which SAE features fire for a text prompt and for an image of that same prompt, and measure whether those sets overlap, layer by layer.

I sent 200 prompts through two Qwen3.5 models, each twice, once as plain text and once as a rendered image of the same text, and compared the active SAE features at each layer.

The answer is mostly no, with a nuance: text and image representations stay largely separate throughout both models. Instruction tuning dramatically improves geometric alignment (cosine similarity), but leaves the discrete feature overlap (Jaccard) almost unchanged - the model points in more similar directions, but activates different features to get there.
...
Why the last token? In a causal language model, the last token is the position from which the next token will be generated. By that point, through all the attention operations above it, its residual stream has aggregated information from every preceding token in the sequence. It is the model's most complete summary of what it just read, and simultaneously the representation it will use to decide what comes next. The SAE features active at the last token therefore capture both the model's contextual understanding of the full input and its implicit "plan" for the upcoming generation. If text and image of the same content are being processed equivalently, that equivalence should be most visible here: two prompts that mean the same thing should leave the model in the same generative state, regardless of whether they arrived as pixels or tokens.
...
**Middle layers (L5–18): slow drift upward.** Both metrics increase but stay low (Jaccard < 5%, cosine < 20%). The IQR band is wide, some prompt pairs accumulate meaningful similarity while most do not. There is no single fusion layer.

**Late layers (L19–23): partial convergence.** Jaccard peaks at ~10% median, cosine at ~20–30%. A Jaccard of 10% means text and image share only ~5 of their top-50 features on average, but it is 70× the random baseline.

### The architecture could explain the spike
...
**Multiple cosine spikes, not one.** The same [3 GatedDeltaNet + 1 Attention] pattern extended to 64 layers places Attention layers at L3, L7, L11, ..., L59, L63. The 27B shows cosine peaks but not on specific Deltanet / Attention swap.
...
| Cosine spikes               | 1 (L18) | Multiple     |
| EN vs FR gap                | None    | None         |
| Topic structure             | None    | None         |

**Cosine is sensitive to post-training (or scale) but Jaccard is not.**
